...
## Phase P0 — shared foundations (no user-visible change) — ✅ DONE (0ee8428)
...
## Phase P1 — preferred systems everywhere on mobile — ✅ DONE (d083300)
...
## Phase P2 — calendar detail screens ×6 — ✅ DONE (62ce8e0)

**Deviation, deliberate:** no second route. The live block
(`components/calendars/live.tsx`) mounts inside the existing
`/learn/[system]` reader for the six calendar keys — same keys, same
bundled prose, and the Today board rows already point there. It is the
mobile twin of the web `CalendarDetail` template: live hero (today in that
...
## Phase P3 — reading systems: Gene Keys, Personality, pair parity — ✅ DONE (ed89729, ca0d192)
...
## Phase P4 — oracles browser — ✅ DONE (3e396db)
...
## Phase P6 — learning library articles — ✅ DONE
...
## Phase P7 — search + favorites — ✅ DONE (9f7b766, 1bbc9ca)
...
## Phase P9 — verification (owed from the last session too) — ✅ DONE

Walked live on AVD `pleiad`; screenshots in
`~/Desktop/pleiad-v2-testing/mobile-v2/`. See
HANDOFF_2026-07-31_MOBILE_V2_PARITY.md §2–3.
...
- **Expo typed routes are generated by `expo start`, not by `expo export`.**
  A new screen fails `tsc` with "not assignable to parameter of type
  '/' | ..." until Metro has run once and rewritten
  `packages/mobile/.expo/types/router.d.ts`. That file was stale from
  22 July, so `/calendar` was missing from the union too.
