...
describe('relationship field demo', () => {
  ...
  it('surfaces distinct evidence from multiple systems for one pair', () => {
    ...
    // Was 'companionship' while the bodygraph wheel was mis-anchored at 0° Aries;
    // with the corrected gates this pair resolves to a split bridge.
    expect(edge?.layers.humanDesign.tie?.type).toBe('split-bridge')
